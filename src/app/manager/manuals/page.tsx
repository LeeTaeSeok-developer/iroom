...
export default function ManagerManualsPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const [productSnapshot, manualSnapshot] = await Promise.all([
      ...
      const manuals = manualSnapshot.docs.map((item) =>
        ManualEntryModel.fromMap(item.data(), item.id)
      );

      const manualMap = new Map<string, ManualEntryModel>();
      for (const manual of manuals) {
        if (manual.productId) {
          manualMap.set(manual.productId, manual);
        }
      }

      const merged: ManualListItem[] = products.map((product) => ({
        product,
        manual: manualMap.get(product.id) ?? null,
      }));

      setItems(merged);
    } catch (error) {
      alert("매뉴얼 목록을 불러오지 못했어요.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteManual = async (item: ManualListItem) => {
    if (!item.manual) {
      alert("삭제할 매뉴얼이 없어요.");
      return;
    }
    ...
    const ok = window.confirm(
      `'${item.product.name}' 제품의 매뉴얼을 삭제할까요?\n삭제 후 되돌릴 수 없어요.`
    );

    if (!ok) return;

    try {
      await deleteDoc(doc(db, "manuals", item.manual.id));
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter(({ product, manual }) => {
      const joined = [
        product.name,
        product.brandName,
        product.category,
  ...
  const manualExistsCount = items.filter((item) => !!item.manual).length;
  const manualActiveCount = items.filter((item) => item.manual?.isActive).length;
      ...
      <div className="relative z-10">
        ...
        <section className="mt-7">
          ...
          <p className="mt-3 text-[15px] leading-7 text-slate-600">
            제품별 사용 가이드, 영상, 소모품, 구성품, 액세서리, FAQ, 사양 정보를 관리할 수 있어요.
          </p>
        </section>
        ...
        <section className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-[24px] border border-slate-200 bg-white px-5 py-4 shadow-sm">
            <p className="text-[12px] font-semibold text-slate-500">매뉴얼 있음</p>
            <p className="mt-2 text-[26px] font-black text-slate-900">
              {manualExistsCount}
            </p>
          </div>
        ...
        <section className="mt-5 rounded-[28px] border border-slate-200 bg-white p-4 shadow-[0_18px_45px_rgba(15,23,42,0.08)] sm:p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button
                type="button"
                onClick={() => router.push("/manager/manuals/new")}
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <div className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-600">
              <BadgeCheck size={14} />
              활성 매뉴얼 {manualActiveCount}개
            </div>

            <div className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-600">
              <BookOpenText size={14} />
              미작성 {totalCount - manualExistsCount}개
            </div>
          </div>
        </section>
          ...
          {loading ? (
            ...
            <div className="rounded-[28px] border border-slate-200 bg-white px-6 py-10 text-center shadow-sm">
              ...
              <p className="mt-2 text-sm text-slate-500">
                검색어를 바꾸거나 새 매뉴얼을 추가해보세요.
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              {filteredItems.map(({ product, manual }) => (
                <div
                  key={product.id}
                      ...
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            >
                              {product.isActive ? "제품 사용 중" : "제품 숨김"}
                            </span>
                            ...
                            >
                              매뉴얼 노출 {product.manualEnabled ? "ON" : "OFF"}
                            </span>
                            ...
                            {manual && (
                              ...
                              >
                                {manual.isActive ? "매뉴얼 사용 중" : "매뉴얼 숨김"}
                              </span>
                            )}
                          </div>
                          ...
                          <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">
                            ...
                            <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1.5 font-medium">
                              <FileText size={13} />
                              FAQ {manual?.faqs?.length ?? 0}개
                            </span>

                            <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1.5 font-medium">
                              <Wrench size={13} />
                              사용가이드 {manual?.usageGuides?.length ?? 0}개
                            </span>

                            <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1.5 font-medium">
                              <Package size={13} />
                              소모품 {manual?.consumables?.length ?? 0}개
                            </span>

                            <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1.5 font-medium">
                              <CircleHelp size={13} />
                              사양 {manual?.specs?.length ?? 0}개
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="flex shrink-0 gap-2">
                          <button
                            ...
                            onClick={() => {
                              if (manual) {
                                router.push(`/manager/manuals/${manual.id}`);
                              } else {
                                router.push(
                                  `/manager/manuals/new?productId=${product.id}`
                                );
                              }
                            }}
                          ...
                          <button
                            type="button"
                            onClick={() => handleDeleteManual({ product, manual })}
                            disabled={!manual}
                            className={`inline-flex items-center gap-2 rounded-[16px] px-4 py-2.5 text-sm font-bold transition ${
                              manual
