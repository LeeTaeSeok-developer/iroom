...
export default function CautionPage({ open, onClose }: Props) {
  ...
  useEffect(() => {
    const fetchCautionPolicy = async () => {
      if (!open) return;

      try {
        setLoading(true);

        const snap = await getDoc(doc(db, "policies", "caution"));
        ...
        console.error("주의사항 불러오기 실패:", error);
        setPolicy(null);
      } finally {
        setLoading(false);
      }
              ...
              <div className="rounded-[24px] bg-white p-5 shadow-[0_8px_24px_rgba(0,0,0,0.08)]">
                ...
                <ul className="mt-4 space-y-2">
                  {lines.map((line, index) => (
                    ...
                    >
                      <span className="mt-[9px] h-[6px] w-[6px] shrink-0 rounded-full bg-red-400" />
                      <span>{line}</span>
                    </li>
                  ))}
                </ul>
